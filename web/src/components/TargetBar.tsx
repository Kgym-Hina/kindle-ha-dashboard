    ...
    <section className="target-bar panel-card">
      ...
      <div className="target-controls">
        <div className="segmented-control" role="group" aria-label="发布目标">
          ...
          <button type="button" className={mode === "zone" ? "is-active" : ""} onClick={() => onModeChange("zone")}>Zone</button>
        </div>
        {mode === "zone" ? (
          <div className="zone-select-wrap">
            <select value={zoneId || ""} onChange={(event) => onZoneChange(event.target.value || null)} aria-label="选择区域">
              <option value="">选择 Home Assistant 区域</option>
              {zones.map((zone) => <option key={zone.zone_id} value={zone.zone_id}>{zone.name}</option>)}
            </select>
            <button className="icon-button" type="button" onClick={onRefreshZones} aria-label="刷新区域" title="刷新区域">↻</button>
          </div>
        ) : (
          <span className="target-note">所有未匹配 Zone 的 Kindle 使用这份界面</span>
        )}
      </div>
    </section>
  );
