...
- 监听 evdev 触摸与物理键，按 JSON 的 `action` 调用 HA 服务、切换页面或弹出消息；
- 支持多级页面、底部返回/主页/设置导航，以及设置页中的刷新配置和退出程序；
- 支持文本实体绑定、开关控制和温控组件；
- 运行时停用 Kindle 原生 framework，拦截物理电源键，并持续保持屏幕唤醒；
...
`input_device` 默认使用 `/dev/input/event1` 触摸屏，`key_device` 默认使用 `/dev/input/event0` 物理键。物理键在子页面返回主页，在主页打开设置页面。
