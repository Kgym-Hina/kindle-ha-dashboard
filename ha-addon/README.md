...
backend 通过 `/data/documents/` 保存每个 portable/房间文档，通过 HA REST API 更新 entity attributes；房间列表通过 HA WebSocket Area Registry 读取。Kindle 端只需要配置 HA 地址和 Long-Lived Access Token，不需要暴露 Add-on 端口给 Kindle。
